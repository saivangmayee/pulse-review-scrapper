...
def _extract_rating(article) -> str:
    """
    Trustpilot markup varies. We try multiple common patterns:
    - aria-label like "Rated 1 out of 5"
    - element with data-service-review-rating
...
def scrape_trustpilot(domain_or_slug: str, start_date: str, end_date: str, max_pages: int = 30) -> list[dict]:
    ...
    Input:
      domain_or_slug like "notion.so" (recommended)

    URL:
      https://www.trustpilot.com/review/<domain_or_slug>?page=2

    Output:
      List of review dicts with fields:
        - title, review, date, rating, reviewer, source
    """
